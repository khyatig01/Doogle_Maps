...
public class CS400Graph<T> implements GraphADT<T> {
	...
	protected class Path implements Comparable<Path> {
		...
		public Path(Path copyPath, Edge extendBy) {
			...
			this.dataSequence.add(this.end.data);
	...
	protected Path dijkstrasShortestPath(T start, T end) {
		...
		// throw NoSuchElementException for vertex not containing start or end
		if (vStart == null || vEnd == null || !vertices.contains(vStart) || !vertices.contains(vEnd)) {
			throw new NoSuchElementException();
		}

		// sub-class with vertex information
		class vertexInfo {
			Vertex vertex;
			Path path;
			...
			vertexInfo(Vertex v) {
				this.vertex = v;
				this.path = null;
				this.cumulativeWeight = Integer.MAX_VALUE;
				this.isVisited = false;
			}
		}

		// Priority Queue initialization for editing comparator
		PriorityQueue<vertexInfo> pqueue = new PriorityQueue<vertexInfo>((v1, v2) -> {
			return v1.path.compareTo(v2.path);
		});

		// create list of all vertices from hashtable of vertices
		java.util.Enumeration<Vertex> verticesList = vertices.elements();

		// create hashtable
		Hashtable<Vertex, vertexInfo> infoTable = new Hashtable<Vertex, vertexInfo>();

		while (verticesList.hasMoreElements()) {
			Vertex vertex = verticesList.nextElement();
			infoTable.put(vertex, new vertexInfo(vertex));
		}

		// initialize start vertex information
		vertexInfo v = infoTable.get(vStart);
		v.cumulativeWeight = 0;
		v.path = new Path(vStart);
		...
		while (!pqueue.isEmpty()) {
			...
			vertexInfo current = pqueue.poll();

			// find all targets to current source vertex
			for (Edge e : current.vertex.edgesLeaving) {

				Vertex n = e.target;
				if (infoTable.get(n).isVisited != true) {
					vertexInfo next = infoTable.get(n);

					// if distance of next can be reduced
					if (next.cumulativeWeight == Integer.MAX_VALUE
							|| next.cumulativeWeight > (current.cumulativeWeight + e.weight)) {

						next.cumulativeWeight = current.cumulativeWeight + e.weight;

						// extend path of next
						next.path = new Path(current.path, e);
		...
		// throw exception if no path if found between source and target vertices
		if (infoTable.get(vEnd).path == null)
			throw new NoSuchElementException();

		// return path of end vertex
		return infoTable.get(vEnd).path;
	}
